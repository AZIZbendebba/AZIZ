    ...
    <section className="section-padding bg-gris-fume">
      <div className="container-site">
        <SectionTitle
          ...
          title="Deux entités,&#10;une seule vision."
          description="Solid Surface Tunisie forment un groupe intégré capable de prendre en charge chaque projet de A à Z , de la surface au meuble, du dessin à la pose."
          align="center"
        />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-gris-fume/50 mt-4">
          {entites.map((entite, i) => (
            ...
            >
              ...
              <h3 className="font-serif font-light text-blanc-pur text-3xl md:text-4xl mb-6">
                {entite.nom}
              </h3>
              <GoldDivider variant="left" className="mb-8" />
              <p className="font-sans font-light text-blanc-pur/60 text-sm leading-relaxed mb-8">
                {entite.description}
              </p>
              <ul className="space-y-3 mb-10">
                {entite.points.map((point) => (
                  <li key={point} className="flex items-center gap-3">
                    ...
                    <span className="font-sans font-light text-sm text-blanc-pur/60">{point}</span>
                  </li>
                ))}
              </ul>
              <Link
                href={entite.href}
                className="font-sans text-[0.65rem] font-medium tracking-[0.15em] uppercase text-or-champagne hover:text-or-clair inline-flex items-center gap-2 transition-colors duration-300 group"
              >
                En savoir plus
        ...
        >
          <p className="font-serif font-light italic text-blanc-pur/60 text-2xl md:text-3xl max-w-3xl mx-auto">
